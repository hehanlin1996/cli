...
- TestTask_TasklistDryRun: validates the dry-run request shapes for `task +tasklist-create --data` and `task +tasklist-task-add --section-guid` without hitting live APIs; this covers flag parsing and rendered POST method/URL/params/body.
...
| ✓ | task +tasklist-create | shortcut | tasklist_workflow_test.go::TestTask_TasklistWorkflowAsBot/create tasklist with task as bot; tasklist_workflow_test.go::TestTask_TasklistWorkflowAsUser/create tasklist as user; tasklist_add_task_workflow_test.go::TestTask_TasklistAddTaskWorkflow; tasklist_dryrun_test.go::TestTask_TasklistDryRun/create tasklist with seeded child tasks | `--name` only; `--name` plus task array in `--data` | |
| ✕ | task +tasklist-members | shortcut |  | none | requires real member open_id fixtures to add, remove, or set tasklist members |
| ✓ | task +tasklist-task-add | shortcut | tasklist_add_task_workflow_test.go::TestTask_TasklistAddTaskWorkflow/add task to tasklist; tasklist_dryrun_test.go::TestTask_TasklistDryRun/add task to tasklist | `--tasklist-id`; `--task-id`; `--section-guid` dry-run | |
